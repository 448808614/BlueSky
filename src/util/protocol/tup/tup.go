...
func (u *UniAttribute) putBase(data interface{}, os *codec.Buffer) error {
	...
	switch data.(type) {
	case int64:
		err = os.WriteInt64(data.(int64), 0)
	case int32:
		err = os.WriteInt32(data.(int32), 0)
	case int16:
		err = os.WriteInt16(data.(int16), 0)
	case int8:
		err = os.WriteInt8(data.(int8), 0)
	case uint32:
		err = os.WriteUint32(data.(uint32), 0)
	case uint16:
		err = os.WriteUint16(data.(uint16), 0)
	case uint8:
		err = os.WriteUint8(data.(uint8), 0)
	case bool:
		err = os.WriteBool(data.(bool), 0)
	case float64:
		err = os.WriteFloat64(data.(float64), 0)
	case float32:
		err = os.WriteFloat32(data.(float32), 0)
	case string:
		err = os.WriteString(data.(string), 0)
	case TarsStructIF:
...
func (u *UniAttribute) getBase(data interface{}, is *codec.Reader) error {
	...
	switch (data).(type) {
	case *int64:
		err = is.ReadInt64(data.(*int64), 0, true)
	case *int32:
		err = is.ReadInt32(data.(*int32), 0, true)
	case *int16:
		err = is.ReadInt16(data.(*int16), 0, true)
	case *int8:
		err = is.ReadInt8(data.(*int8), 0, true)
	case *uint32:
		err = is.ReadUint32(data.(*uint32), 0, true)
	case *uint16:
		err = is.ReadUint16(data.(*uint16), 0, true)
	case *uint8:
		err = is.ReadUint8(data.(*uint8), 0, true)
	case *bool:
		err = is.ReadBool(data.(*bool), 0, true)
	case *float64:
		err = is.ReadFloat64(data.(*float64), 0, true)
	case *float32:
		err = is.ReadFloat32(data.(*float32), 0, true)
	case *string:
		err = is.ReadString((data).(*string), 0, true)
	case TarsStructIF:
